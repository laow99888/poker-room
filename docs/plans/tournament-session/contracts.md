...
## 2026-09-19 快捷弃牌与估算扩展（优先于下文旧限制）

- `POST /api/table/quick-fold`：只读v2接口，接收冻结context、hand_id、hero_cards及ops。ops为空时不要求底牌；只用PokerKit完成前注和盲注记账。已有动作则验证并回放原记录，须已弃牌或轮到英雄合法弃牌；全下、未知英雄结果、非法动作拒绝。返回可证明的余额行，不分配未知赢家。
- 用户点击“本手弃牌并下一手”代表现场已到下一手。`closeQuickFold`将本手标为`manual_close`且`quickFold=true`；保留原handId和已录入ops，英雄余额用引擎验证值，其他未知行沿用冻结手前余额并标`source='estimated'`。没有记录的下注不自行推测。
- 普通结算保留原校验，可显式“未填对手沿用估算”；只填空的对手行，不覆盖已有输入，不替英雄虚构未知摊牌结果。估算延续不额外扣未知对手的盲注/前注，避免在不知道获池者时逐手漏掉底池金额；该值是参考值而非实际账本。
- 只要仍有估算行，就不强制全桌差额原因/确认，差额不作为真实盈亏。所有行都改成真实值后恢复原差额校验。估算0不得作为淘汰依据，淘汰需要实际余额或显式人员变动。
- `Occupant.chipsEstimated`跨手保存。已知投入只能更新数值，不能把原本估算的起始余额变成已核实结果。手工校准单个玩家只清除该玩家估算标记。
- `HandSummary`增加可选`quickFold`和`estimatedOccupantIds`；旧数据缺省分别为false/空数组。`finalChipsByOccupant`中估算成员不能计作真实结果。`manual_close`或含估算余额的手不创建学习任务。
- 选牌前可`calibrateStartingChips`修改某个正数手前余额，同步本手context并增加revision，保留handId及历史。开始选牌/录动作后不得改冻结起始数据；0余额淘汰、进出/换座仍通过手间结算编辑。
- 快捷提交复用现有prepare、事务指纹、原子保存和多标签冲突检查。请求失败、保存失败、旧响应不能推进手数或再次扣款。常规名单自动轮转；空庄位、空小盲、实际人员变更仍需校准。
- 此扩展不支持大盲前注BBA，仍是每人前注。schemaVersion保持1，增加字段兼容现有存档；旧版程序不应继续打开已使用此功能的新存档。

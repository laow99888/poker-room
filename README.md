...
- **锦标赛连续牌桌（主界面）**：建桌选择 6/8/9 座，默认坐满，你固定在画面下方，只需点选庄位；实际参与人数支持 2–9 人。逐手自动轮转庄位与盲位，
  可一键“本手弃牌并下一手”，自动记录自己的盲注、前注和已录入投入；未知对手余额沿用估算，无需每手逐人核对。筹码可选校准，支持人员进出、升盲、单挑、
  前注与决赛桌 ICM；手机优先动作录入。设计与验收详见
  `docs/plans/tournament-session/`（含 RESULTS.md 与 evidence/ 截图）；
...
- **对手画像**：默认关闭并隐藏代号布局；高级设置开启后，从下一手开始累计完整牌谱并使用画像；
